...
try {
  ...
  try {
    ...
    try {
      $verifyVote = "SELECT COUNT(user_id) AS row_count FROM user_like WHERE user_id = {$user_id} AND post_id = {$post_id}";
      $row = $conx->query($verifyVote)->fetchAll();
      $row_count = intval($row[0]['row_count']);
      
      if ($row_count > 0) {
        $updateLike = "UPDATE user_like SET upvolt = 0, downvolt = 1 WHERE user_id = {$user_id} AND post_id = {$post_id}";
        
      } else {
        $updateLike = "INSERT INTO user_like(user_id,post_id,downvolt) VALUES ({$user_id},{$post_id},1)";
      }
      ...
      $status =200;
      $response= ['msg' => 'sucess', 'query'=> $updateLike,'queryVefify'=>$row];
